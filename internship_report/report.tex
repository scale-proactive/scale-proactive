...
Actually, the execution time is here the response time of the request. To perform this evaluation, we average execution times of requests by batch of 50 consecutive requests of the same group. Figure~\ref{fig:effectivity} depicts the results of prioritized and not prioritized executions:
...
PARLER DU TEMPS D'EXEC The average execution time of an empty request is 1471303 ns, which is 1.4 ms (the time to poll the request and to execute it in a thread
...
\begin{figure}[!ht]
      ...
      \caption{Insertion time of sequential requests belonging to all parts of a complex priority graph}
      \label{fig:sequential} 
\end{figure}
...
With the matrix, we reach the same performance as with the integer based specification, because we reduce the overtakability problem to a fundamental operation.

AVANTAGE RAPIDE
INCONVENIENT MEMOIRE
