 ...
 * They are generated by a <I>stub object</I>, whose role is to act as a
 * representative for the reified object.
 * 
...
public class MethodCall implements java.io.Serializable, Cloneable {
	...
	/**
	 * The hashtable that caches Method/isAsynchronousCall This dramatically
	 * improves performances, since we do not have to call isAsynchronousCall
	 * for every call, but only once for a given method
	 */
	...
	/**
	 * byte[] to store effectiveArguments. Required to optimize multiple
	 * serialization in some case (such as group communication) or to create a
	 * stronger asynchronism (serialization of parameters then return to the
	 * thread of execution before the end of the rendez-vous).
	 */
	...
	/**
	 * Sets recycling of MethodCall objects on/off. Note that turning the
	 * recycling off and on again results in the recycling pool being flushed,
	 * thus damaging performances.
	 * 
	 * @param value
	 *            sets the recycling on if <code>true</code>, otherwise turns it
	 *            off.
	 */
	...
	/**
	 * Indicates if the recycling of MethodCall objects is currently running or
	 * not.
	 * 
	 * @return <code>true</code> if recycling is on, <code>false</code>
	 *         otherwise
	 */
	 ...
	 * 
	 * @param reifiedMethod
	 *            a <code>Method</code> object that represents the method whose
	 *            invocation is reified
	 * @param effectiveArguments
	 *            the effective arguments of the call. Arguments that are of
	 *            primitive type need to be wrapped within an instance of the
	 *            corresponding wrapper class (like
	 *            <code>java.lang.Integer</code> for primitive type
	 *            <code>int</code> for example).
	 * @return a MethodCall object representing an invocation of method
	 *         <code>reifiedMethod</code> with arguments
	 *         <code>effectiveArguments</code>
	 */
	public synchronized static MethodCall getMethodCall(Method reifiedMethod,
		...
		return new MethodCall(reifiedMethod, genericTypesMapping, effectiveArguments,
				exceptioncontext);
	}
	...
	public synchronized static MethodCall getMethodCall(Method reifiedMethod,
			...
			Map<TypeVariable<?>, Class<?>> genericTypesMapping) {
		MethodCallExceptionContext exceptioncontext = ExceptionHandler
				.getContextForCall(reifiedMethod);
		return getMethodCall(reifiedMethod, genericTypesMapping, effectiveArguments,
				exceptioncontext);
	}
	...
	public synchronized static MethodCall getComponentMethodCall(Method reifiedMethod,
			...
			Map<TypeVariable<?>, Class<?>> genericTypesMapping, String interfaceName,
			ItfID senderItfID) {
		return MethodCall.getComponentMethodCall(reifiedMethod, effectiveArguments,
				genericTypesMapping, interfaceName, senderItfID,
				ComponentRequest.STRICT_FIFO_PRIORITY);
	}

	/**
	 * Tells the recycling process that the MethodCall object passed as
	 * parameter is ready for recycling. It is the responsibility of the caller
	 * of this method to make sure that this object can safely be disposed of.
	 */
	...
	/**
	 * Builds a new MethodCall object. Please, consider use the factory method
	 * <code>getMethodCall</code> instead of build a new MethodCall object.
	 */
	...
	public MethodCall(Method reifiedMethod,
		...
		this.genericTypesMapping = ((genericTypesMapping != null) && (genericTypesMapping
				.size() > 0)) ? genericTypesMapping : null;
	 ...
	 * Fields of the object are not copied. Please, consider use the factory
	 * method <code>getMethodCall</code> instead of build a new MethodCall
	 * object.
	 * 
	 ...
	 *             thrown if the reflection of the call failed.
	 * @throws InvocationTargetException
	 *             thrown if the execution of the reified method terminates
	 *             abruptly by throwing an exception. The exception thrown by
	 *             the execution of the reified method is placed inside the
	 *             InvocationTargetException object.
	 * @return the result of the invocation of the method. If the method returns
	 *         <code>void</code>, then <code>null</code> is returned. If the
	 *         method returned a primitive type, then it is wrapped inside the
	...
	public Object execute(Object targetObject) throws InvocationTargetException,
		...
		try {
			...
			// In order to call from this class protected methods of the Active
			// Object,
			...
			e.printStackTrace();
			throw new MethodCallExecutionFailedException("Arguments for the method "
					+ this.getName() + " are invalids: " + e + "for the object "
					+ targetObject + "(" + targetObject.getClass().getName() + ")", e);
		}
	}
	...
	private Class<?>[] fixBugRead(FixWrapper[] para) {
		...
		for (int i = 0; i < para.length; i++) {
			// System.out.println("fixBugRead for " + i + " value is "
			// +para[i]);
	...
	private FixWrapper[] fixBugWrite(Class<?>[] para) {
		...
		for (int i = 0; i < para.length; i++) {
			// System.out.println("fixBugWrite for " + i + " out of " +
			// para.length + " value is " +para[i] );
	...
	// build a key for uniquely identifying methods, including parameterized
	// ones
	private static String buildKey(Method reifiedMethod,
			Map<TypeVariable<?>, Class<?>> genericTypesMapping) {
		// TODO It seems genericTypesMapping is always an empty map, It is
		// either useless or not correctly built.
		final StringBuffer sb = new StringBuffer(
				(reifiedMethod.getDeclaringClass().getName()));
		...
		if ((genericTypesMapping != null) && (!genericTypesMapping.isEmpty())) {
			for (int i = 0; i < parameters.length; i++) {
				sb.append((genericTypesMapping.containsKey(parameters[i])) ? genericTypesMapping
						.get(parameters[i]).getName() : parameters[i]);
			}
		} else {
			for (int i = 0; i < parameters.length; i++) {
	...
	protected void readTheObject(java.io.ObjectInputStream in)
		...
		if (this.reifiedMethod == null) {
			// Reads several pieces of data that we need for looking up the
			// method
	...
	/**
	 * Returns a boolean saying whether the method is one-way or not. Being
	 * one-way method is equivalent to
	 * <UL>
	 * <LI>having <code>void</code> as return type
	 * <LI>and not throwing any checked exceptions
	 * </UL>
	 * . If the caller asks for a RuntimeException, then the call is not one
	 * way.
	 * 
	...
	static class ReifiableAndExceptions {
		...
		String reason; // Why is the method synchronous ? null if it is
						// asynchronous
	}
		...
		if (!cached.reifiable) {
			...
			if (cached.exceptions) {
				if (getExceptionContext().isExceptionAsynchronously()) {
					/*
					 * ProActive.tryWithCatch() is used, so this call is
					 * asynchronous
					 */
	...
	private ReifiableAndExceptions getCachedMethodAnalysis(Method m) {
		...
		if (cached == null) {
			...
			if (m.getReturnType().equals(java.lang.Void.TYPE)) {
				...
				/*
				 * void is reifiable even though the check by the MOP would tell
				 * otherwise
				 */
				...
				try {
					if ((getGenericTypesMapping() != null)
							&& getGenericTypesMapping().containsKey(
									m.getGenericReturnType())) {
						// if return type is a parameterized type, check with
						// actual parameterizing type
						MOP.checkClassIsReifiable(getGenericTypesMapping().get(
								m.getGenericReturnType()));
					} else {
	...
	/**
	 * Checks if the <code>Call</code> object can be processed with a future
	 * semantics, i-e if its returned object can be a future object.
	 * 
	 * Two conditions must be met :
	 * <UL>
	 * <LI>The returned object is reifiable
	 * <LI>The invoked method does not throw any exceptions or they are catched
	 * asynchronously
	 * </UL>
	 * 
